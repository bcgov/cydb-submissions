...
CREATE UNIQUE INDEX `rejection_reasons_text_unique` ON `rejection_reasons` (`text`);--> statement-breakpoint
...
CREATE TABLE `__new_submissions` (
	...
	`decided_by` text,
	`decided_at` text,
	CONSTRAINT "submissions_status_check" CHECK("__new_submissions"."status" IN ('submitted','OCR queued','OCR Error','OCR processed','ready for review','ready for clinician','reviewed','accepted','rejected','invalid')),
...
CREATE INDEX `submissions_search_indexed_idx` ON `submissions` (`search_indexed_at`);--> statement-breakpoint
INSERT OR IGNORE INTO rejection_reasons (text, active) VALUES
 ('Incomplete information', 1),
 ('Missing required documentation', 1),
 ('Does not meet eligibility criteria', 1),
 ('Duplicate submission', 1),
 ('Illegible or corrupted attachment', 1);
