...
export interface ChesTokenOpts {
	tokenUrl: string;
	clientId: string;
	clientSecret: string;
	scope?: string;
...
export class ChesTokenCache {
	private cached: CachedToken | null = null;
	private inflight: Promise<string> | null = null;

	constructor(private opts: ChesTokenOpts) {}
	...
	private async fetchNew(): Promise<string> {
		...
		if (!res.ok) {
			...
			throw new Error(`CHES token endpoint failed: ${res.status} ${text.slice(0, 200)}`);
		}
