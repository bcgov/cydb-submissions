...
import { getRevokedRoles, type DrizzleDb } from './roles';
...
export async function applyBypass(
	...
	const target =
		identities.find((i) => i.email === requestedEmail) ?? (requestedEmail ? null : identities[0]);
	...
	const revoked = await getRevokedRoles(db, row.id);
	const effective = new Set(target.roles.filter((r) => !revoked.has(r)));
